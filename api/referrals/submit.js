...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Minimal validation (aligns with your form)
    ...
    // Email (best effort; skip if not fully configured)
    if (
      ...
      try {
        ...
        await resend.emails.send({
          from: process.env.EMAIL_FROM, // verified sender (Resend)
          to: [process.env.BDM_EMAIL_TO], // BDM recipient
          ...
          `.trim(),
          reply_to: row.referrer_email || undefined, // lets BDM reply to referrer
        });
      } catch {
        // Silently ignore email errors (don’t block the successful insert)
      }
    }
